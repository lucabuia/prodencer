...
#     # sg_tetragonal = 376
#     O_coords = (atomic_positions[1,:]+atomic_positions[2,:])/2
#     radius = 1.41
...
#     coeffs = pd.project_sphere(proj_charge, lattice, atomic_positions[2,:], 1.41)
...
# plt.plot(x_vals, GM4m, marker="o", linestyle="-", color="r", label=r"$\Gamma_4^-$")
# plt.plot(x_vals, O_pz_GM4m, marker="o", linestyle="-", color="b", label=r"$\Gamma_4^- \rightarrow p_z$")
...
plt.ylabel(r'Interatomic dipole [|e|$\AA$]')
plt.grid(False); plt.xlim(32, 32.16)
plt.tight_layout()
# plt.legend()
# plt.ylim(0,0.03)
plt.xticks(np.arange(32, 32.17, step=0.04))
plt.yticks(np.arange(-5.39, -5.36, step=0.01))
plt.show()