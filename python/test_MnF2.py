...
positions_Mn = np.round(wyckoff(center_Mn, 419),5)
positions_F = np.round(wyckoff(center_F, 419),5)
...
for idx, pos in enumerate(positions_Mn):
    ...
    coeffs_row = project_sphere(mx, lattice, np.array(pos), radius_Mn)
...
# output_analytical_densities(lattice, positions_Mn, radius_Mn, coeffs, "MnF2_2a_Mx")




# Get irreps at Gamma
positions = np.concatenate((positions_Mn, positions_F), axis=0, out=None, dtype=None, casting="same_kind")
numbers = [0,0,1,1,1,1] # Mn Mn F F F F
kpoint = [0,0,0]

irreps, rotations, translations, mapping_little_group = get_spacegroup_irreps(lattice, positions, numbers, kpoint)
